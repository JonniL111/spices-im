document.addEventListener("DOMContentLoaded", function() {
  ...
  $('.supermenu__item').hover(function () {
  ...
  $('.header-search').hover(function () {
    ...
    slidesToScroll: 1,
    slidesToShow: 4,
    prevArrow: '<button class="slick-btn product-slider__slider-btn product-slider__slider-btnprev" ></button>',
    nextArrow: '<button class="slick-btn product-slider__slider-btn product-slider__slider-btnnext" ></button>',
    responsive: [
      {
        breakpoint: 1140,
        settings: {
  ...
  if($(".section-2").length){
   ...
   const innerHeight = window.innerHeight;
    $(window).on('scroll',function () {
      ...
      $(".section-1")
        .css("background-position","50% " + Math.floor((scrollTop / 6 )) + "px");
      $(".section-2")
        .css("background-position","50% " + Math.floor(-(offsetTop-scrollTop-innerHeight) / 6-100 ) + "px");
    }); 
  }
...
/* slick */
  ...
  $(document).mouseup(function (e){ // событие клика по веб-документу
    ...
    let button = $(".mobile-menu__button");
		if (!elem.is(e.target) //закрываем блок если было нажато на пустом поле, ф не на блоке или дочернем эллементе
